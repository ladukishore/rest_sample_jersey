    ...
    protected Date fromDateQueryParameter(final String dateParam) {
        if (dateParam != null) {
            try {
                ...
                throw new WebApplicationException(Response.status(Status.BAD_REQUEST).entity("Unable to parse date querystring parameter " + dateParam).build());
            }
        } else {
            return null;
        }
